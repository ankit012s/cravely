...
public class DataSeeder {
    ...
    CommandLineRunner seed(RestaurantRepository rr, MenuItemRepository mr) {
        ...
        restaurant.setName(name); restaurant.setCuisine(cuisine); restaurant.setAddress(address);
        restaurant.setImageUrl(imageUrl); restaurant.setActive(true); restaurant = rr.save(restaurant);
        Restaurant saved = restaurant;
        dishes.forEach(dish -> { if (mr.findByRestaurantIdAndAvailableTrue(saved.getId()).stream()
            .noneMatch(item -> dish.name().equals(item.getName()))) {
            MenuItem item = new MenuItem(); item.setName(dish.name()); item.setDescription(dish.description());
            item.setPrice(dish.price()); item.setImageUrl(dish.imageUrl()); item.setRestaurant(saved); mr.save(item);
        }});
        }
