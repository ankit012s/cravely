...
public class OrderController {
    ...
    public OrderController(FoodOrderRepository orders,UserRepository users,
        ...
        this.orders=orders;this.users=users;this.restaurants=restaurants;this.menu=menu;
    }

    public record ItemRequest(Long menuItemId,int quantity){}
    public record OrderRequest(Long restaurantId,String deliveryAddress,String paymentMethod,List<ItemRequest> items){}
    ...
    public OrderResponse create(@RequestBody OrderRequest req, Authentication auth){
        ...
        o.setCustomer(u); o.setRestaurant(r);
        ...
        for(ItemRequest ir:req.items()){
            ...
            if(ir.quantity()<=0) throw new IllegalArgumentException("Invalid quantity");
            OrderItem oi=new OrderItem();
            oi.setOrder(o); oi.setMenuItem(mi); oi.setQuantity(ir.quantity());
            oi.setUnitPrice(mi.getPrice());
            o.getItems().add(oi);
            total += mi.getPrice()*ir.quantity();
        }
        o.setTotalAmount(total);
        if("TEST".equalsIgnoreCase(req.paymentMethod())) o.setPaymentStatus("PAID");
        else if("COD".equalsIgnoreCase(req.paymentMethod())) o.setPaymentStatus("PENDING");
